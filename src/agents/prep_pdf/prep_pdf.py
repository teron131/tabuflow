"""Tool-using prep_pdf stage for iterative local data extraction."""
...
from .state import PrepPdfDecision, PrepPdfOutput

PREP_PDF_STAGE_RECURSION_LIMIT = 12
...
def collect_prep_trial_result(result: dict[str, Any]) -> PrepTrialResult:
    ...
    for message in result.get("messages", []):
        ...
        if tool_name != "extract_pdf" or not tool_payload:
            continue
    ...
    if structured_response is not None:
        decision = PrepPdfDecision.model_validate(structured_response)
...
def _prepared_output(
    *,
    ...
    """Build the successful prep_pdf stage output."""
...
def _stopped_output(
    *,
    ...
    """Build the prep_pdf stage output for non-retryable stop decisions."""
    decision = trial.decision
...
def _exhausted_output(
    *,
    ...
    """Build the prep_pdf stage output after all retry trials are exhausted."""
    final_decision = trial.decision
    final_error = (
        final_decision.last_error
        if final_decision and final_decision.last_error
        else trial.last_error or (final_decision.summary if final_decision else None) or f"prep_pdf stage exhausted {safe_max_prep_trials} trial(s) without a usable extraction."
    )
    return PrepPdfOutput(
        status="error",
        extraction_results=trial.extraction_results,
        ...
        trace=append_stage_trace(trace, PREP_PDF_STAGE, f"exhausted {safe_max_prep_trials} trial(s)"),
    )


class PrepPdf(ApplicationAgent):
    """Use prep_pdf tools iteratively to decide the final extraction shape."""
    ...
    def build_graph(
        ...
        *,
        ...
        """Build the compiled prep_pdf stage graph."""
        return create_agent(
            model=self.llm,
            tools=make_pdf_tools(root_dir=self.root_dir),
            system_prompt=PREP_PDF_STAGE_SYSTEM_PROMPT,
            response_format=ToolStrategy(PrepPdfDecision),
    ...
    def invoke(
        ...
        *,
        ...
        """Run the prep_pdf stage in bounded trials and normalize the final outputs."""
        ...
        if last_trial is None:
            return PrepPdfOutput(
                status="error",
                last_error="prep_pdf stage did not run.",
                trace=append_stage_trace(trace, PREP_PDF_STAGE, "failed before starting"),
            )
