...
`pdf inspect` is profile-first: by default it returns `path`, `page_count`, selected 2x2 `overview_batches`, a lightweight `overview_batch_index`, and `profile` layout signatures with suggested visual sample pages. It omits heavy page details by default; pass a page window when focused page text, row geometry, table detections, and table region hints are needed. `pdf prepare` copies the source PDF, renders every page, and creates a normalized-filename workspace with `manifest.json`, `pages/*.jpg`, `text/*.txt`, and `work/` under the root-owned `artifacts/pdf/...` path.
...
- default 2x2 overview batches as required evidence for layout, table count, and continuation decisions, with focused page images for ambiguous pages,
...
- confirm the relevant rendered page overview images were checked before accepting, pruning, or naming PDF table artifacts,
- confirm focused page images were checked for ambiguous PDF table boundaries, same-page repeated tables, low-confidence output, generic columns, or count mismatches,
